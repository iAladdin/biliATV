...
const USER_AGENT = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.99 Safari/537.36';
...
function GetSign(params,appkey,AppSecret=None){
    ...
    if (!AppSecret){
        ...
        return data+ MD5(data+AppSecret);
    }
}

function getCid(aid){
    const url = `http://www.bilibili.com/widget/getPageList?aid=${aid}`
    const info = jsonParse(getStringFromURL(url));
    return info[0];
}
function api_req(cid,quality){
    let ts = Date.now();
    // if not bangumi:
...
function getBilibiliURL(aid,pid,cb) {
    const cid_args = getCid(aid);
    console.log(cid_args);
...
function getAvData(id,page,cb){
    getBilibiliURL(id,page,cb);
